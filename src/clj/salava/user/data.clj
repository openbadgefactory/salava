(ns salava.user.data
  (:require
    ...
    [salava.core.util :as util :refer [replace-nils]]
    [clj-pdf.core :as pdf]
...
(defn events-helper [ctx event user-id]
  (let [badge-info (map (fn [b]
                          {:name (:name b)})
                        (filter #(= (:default_language_code %) (:language_code %)) (select-multi-language-badge-content {:id (:object event)} (into {} (util/get-db ctx)))))
        user-info (u/user-information ctx (:object event))
        user-badge-info (first (filter #(= (:default_language_code %) (:language_code %)) (:content (b/fetch-badge ctx (:object event)))))
    ...
    (cond
      (and (= (:verb event) "follow") (= (:type event) "badge")) {:object_name (or (:name (first badge-info)) "-")}
      (and (= (:verb event) "follow") (= (:type event) "user")) {:object_name (if (and (blank? (:first_name user-info))(blank? (:last_name user-info))) "-" (str (:first_name user-info) " " (:last_name user-info)))}
      (and (= (:verb event) "congratulate") (= (:type event) "badge")) {:object_name (or (:name user-badge-info) "-")}
      (and (= (:verb event) "message") (= (:type event) "badge")) {:object_name (or (:name badge-info) "-") :message (first message)}
      (and (= (:verb event) "publish") (= (:type event) "badge")) {:object_name (or (:name user-badge-info) "-")}
      (and (= (:verb event) "unpublish") (= (:type event) "badge")) {:object_name (or (:name user-badge-info)  "-")}
      (and (= (:verb event) "publish") (= (:type event) "page")) {:object_name (or (:name page-info) "-")}
      (and (= (:verb event) "unpublish") (= (:type event) "page")) {:object_name (or (:name page-info) "-")}
      :else nil
      )))
...
(defn all-user-data
  ([ctx user-id current-user-id]
   (let [all-user-info (u/user-information-and-profile ctx user-id current-user-id)
     ...
     (assoc (replace-nils all-user-info)
       :emails (replace-nils email-addresses)
       :user_badges  (replace-nils user-badges)
       :user_pages (replace-nils user-pages)
       :user_files (replace-nils (:files user-files))
       :events  events-with-info
       :connections (replace-nils connections)
       :pending_badges (replace-nils pending-badges)
       :user_followers (replace-nils user-followers)
       :user_following (replace-nils user-following))))
