...
(defn get-badge-adverts [ctx country tags name issuer-name order id user-id show-followed-only]
  (let [where-params (badge-adverts-where-params country name issuer-name id)
        ...
        order (cond
                ...
                :else "") 
        query (str "SELECT DISTINCT ba.id, ba.country, bc.name, ba.info,
                     ba.criteria_url, bc.image_file, ic.name AS issuer_content_name,
                     ic.url AS issuer_content_url,GROUP_CONCAT( bct.tag) AS tags, ba.mtime,
                     ba.not_before, ba.not_after, ba.kind, ba.application_url,
                     ba.application_url_label, IF(scba.user_id, true, false) AS followed
