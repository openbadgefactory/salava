...
(defn parse-linkedin-user [ctx linkedin-user]
  (let [;location (or (get-in linkedin-user [:location :country :code]) (get-in ctx [:config :user :default-country]))
        ...
        location (-> (get user-locale :country (get-in ctx [:config :user :default-country]))
                     (upper-case))
        language (get user-locale :language (get-in ctx [:config :user :default-language]))
        firstName (get linkedin-user [:firstName :localized (keyword (str language "_" location))] (-> linkedin-user :firstName :localized vals first))
        lastName (get linkedin-user [:lastName :localized (keyword (str language "_" location))] (-> linkedin-user :lastName :localized vals first))
        picture-url (get-in linkedin-user [:profilePicture :displayImage])
        user-info {:first_name firstName
...
(defn linkedin-login [ctx code state current-user-id error]
  (try+
    ...
    (let [access-token (linkedin-access-token ctx code linkedin-login-redirect-path)
          linkedin-user (d/oauth-user ctx "https://api.linkedin.com/v2/me?projection=(id,firstName,lastName,profilePicture)" {:oauth-token access-token} parse-linkedin-user) #_(d/oauth-user ctx "https://api.linkedin.com/v1/people/~:(id,first-name,last-name,picture-url,email-address,location)?format=json" {:oauth-token access-token} parse-linkedin-user)
          t (keyword "handle~")
          email (-> (d/oauth-request :get "https://api.linkedin.com/v2/emailAddress?q=members&projection=(elements*(handle~))" {:oauth-token access-token}) :elements first t :emailAddress)
