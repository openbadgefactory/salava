Loadmore (ns salava.social.ui.stream
    ...
    [:div#advert-event {:class "media message-item"}
     ...
     [:div.media-body
      ...
      [:a {:href "#"
           :on-click #(do
                        (.preventDefault %)
                        (mo/open-modal [:application :badge] {:id subject :state state}))} (t :social/Loadmore)]]]))
...
(defn message-event [event state]
  (let [{:keys [subject verb image_file message ctime event_id name object]}  event
        ...
        ;(str  (t :social/Loadmore) (if (pos? new-messages) (str " (" new-messages " " (if (= 1 new-messages) (t :social/Newmessage) (t :social/Newmessages)) ")")))
...
(defn empty-stream []
  [:div {:class "media message-item tips"}
   ...
   [:div.media-body
    [:i {:class "fa fa-lightbulb-o"}]
    [:div
     [:p.media-heading (str (t :social/Emptystreamheader) " " (t :social/Sometips))]
     [:div.media-body
      [:ul
