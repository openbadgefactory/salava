...
(defn assertion-content [assertion]
  (fn []
          ...
          (for [prop-2 (keys val-1)
            ...
            (if (map? val-2)
              (for [prop-3 (keys val-2)
                ...
                (if-not (map? val-3)
                  [:tr {:key (str name prop-3) :class (str "tr-" prop-1 "-" prop-2 "-" prop-3)}
                   [:td (name prop-1) " " (name prop-2) " " (name prop-3)]
                   [:td (or val-3 "-")]]))
              [:tr {:key (str name prop-2) :class (str "tr-" prop-1 "-" prop-2)}
               [:td (str (name prop-1) " " (name prop-2))]
               [:td  (or (str val-2) "-")]
               ]))
          [:tr {:key (str name prop-1) :class (str "tr-" prop-1)}
           [:td (name prop-1)]
           [:td (or (str val-1) "-")]
           ])))]))
