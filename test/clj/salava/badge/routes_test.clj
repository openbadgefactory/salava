...
(t/deftest-ctx routes-test [ctx]
  (testing "GET /export:"
    (testing "user must be logged in to export badges"
      (let [response (t/test-api-request ctx :get "/app/obpv1/badge/export")]
        (is (= 401 (:status response)))))

    (testing "user has a badge which can be exported"
      (let [{:keys [status body]} (t/test-api-request ctx :get "/app/obpv1/badge/export" {:user test-user})]
        (is (= 200 status))
        (is (= 1 (count (:badges body)))))))

  (testing "GET /badge"
    (testing "user must be logged in"
      (let [response (t/test-api-request ctx :get "/app/obpv1/badge")]
        (is (= 401 (:status response)))))

    (testing "user has two badges"
      (let [{:keys [status body]} (t/test-api-request ctx :get "/app/obpv1/badge" {:user test-user})]
        (is (= 200 status))
        (is (= 2 (count body)))))

    (testing "user has no badges"
      (let [{:keys [status body]} (t/test-api-request ctx :get "/app/obpv1/badge" {:user user-with-no-badges})]
        (is (= 200 status))
        (is (= 0 (count body)))))
    ...
    (testing "badge can be accepted"
      (let [{:keys [status body]} (t/test-api-request ctx
                                                      :post
                                                      (str "/app/obpv1/badge/set_status/" badge-id)
                                                      {:user test-user
