...
SELECT verification_key, activated FROM user AS u
       JOIN user_email AS ue ON ue.user_id = u.id
...
UPDATE user_email SET primary_address = 1, mtime = UNIX_TIMESTAMP() WHERE user_id = :user_id AND email = :email

--name: update-other-addresses!
UPDATE user_email SET primary_address = 0, mtime = UNIX_TIMESTAMP() WHERE user_id = :user_id AND email != :email

--name: update-verify-email-address!
UPDATE user_email SET verified = 1, mtime = UNIX_TIMESTAMP() WHERE user_id = :user_id AND email = :email
