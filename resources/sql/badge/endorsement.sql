...
SELECT se.subject, se.verb, se.object, se.ctime, se.type, seo.event_id, seo.last_checked,requester.id AS requester_id, requester.first_name, requester.last_name, requester.profile_picture, requester.profile_visibility,
    bc.name, bc.image_file, seo.hidden, uber.id, uber.user_badge_id,uber.content, uber.status, uber.mtime
FROM social_event_owners AS seo
INNER JOIN social_event AS se ON seo.event_id = se.id
...
requester.id AS requester_id, requester.profile_picture, requester.first_name, requester.last_name, bc.name, bc.image_file, bc.description
...
SELECT issuer_id FROM user_badge_endorsement_request WHERE id = :id
